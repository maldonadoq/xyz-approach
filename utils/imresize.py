...
def fix_scale_and_size(input_shape, output_shape, scale_factor):
    ...
    if output_shape is not None:
        output_shape = list(np.uint(np.array(output_shape))) + \
            list(input_shape[len(output_shape):])
...
def contributions(in_length, out_length, scale, kernel, kernel_width, antialiasing):
    ...
    field_of_view = np.squeeze(np.uint(np.expand_dims(
        left_boundary, axis=1) + np.arange(expanded_kernel_width) - 1))
...
def resize_along_dim(im, dim, weights, field_of_view):
    ...
    weights = np.reshape(weights.T, list(
        weights.T.shape) + (np.ndim(im) - 1) * [1])
